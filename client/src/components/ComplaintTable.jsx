...
const ComplaintTable = ({ data, user, toggle }) => {
  ...
  return (
    <div className="w-full">
      ...
      <div className="hidden md:grid grid-cols-15 gap-2 px-4 md:px-6 *:py-3 bg-neutral-600 border-b border-gray-100 text-white">
        ...
        <p className="col-span-3 font-bold text-[11px] uppercase tracking-wider">{isRegular ? "Location" : "Client"}</p>
      ...
      <div className="divide-y divide-black">
        {data?.map((complaint, i) => {
          ...
          return (
            <div
              key={complaint._id}
              className="grid grid-cols-1 md:grid-cols-15 gap-2 px-4 md:px-6 py-4 items-center bg-neutral-200 hover:bg-neutral-200/70 transition-colors"
            >
              {/* Complaint Number */}
              ...
              {/* assigned  */}
              <Link to={isRegular ? `/location/${complaint.location?._id}` : `/complaint/${complaint._id}`} className="col-span-2">
                <p className="text-[10px] font-bold text-gray-400 md:hidden uppercase mb-1">Assigned to</p>
              ...
              <Link to={isRegular ? `/location/${complaint.location?._id}` : `/complaint/${complaint._id}`} className="col-span-3 text-sm text-gray-700 font-medium">
                <p className="text-[10px] font-bold text-gray-400 md:hidden uppercase mb-1">{isRegular ? "Location" : "Client"}</p>
                {user?.role === "Admin" ? (
